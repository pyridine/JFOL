...
(define expandable? 
  (lambda (pform)
    (or
     ...
     (pf-is-type? pform pf-not-sig pf-not-sig)    ;;trivial expansion 1: not not
     (and
      (pf-is-type? pform pf-not-sig)
...
(define get-resolution-pivots-unidirectional
  (lambda (lefts rights forbid)
    ...
    (let ((leftnegs (elements-satisfying (lambda (p) (pf-is-type? p pf-not-sig)) lefts)))
    ...
    (let ((disj (disjunctive-components pform))
	  ...
	  (if (null? conj)
		  ...
		  (let ((pform2 (get-single-expression pform)))
		    (cond
		     ((pf-is-type? pform2 pf-not-sig) (get-single-expression pform2))
		     ((equal? atomic-true pform2) atomic-false)
		     ((equal? atomic-false pform2) atomic-true)
		     (else nil)))
		  nil)
	      conj)
	  disj))))
...
(define determine-expansion-type
  (lambda (pform)
    (cond
     ((pf-is-type? pform pf-not-sig pf-not-sig) expansion-type-nn)
     ((and
       (pf-is-type? pform pf-not-sig)
       (equal? atomic-false (get-single-expression pform)) expansion-type-nftt ))
     ((and
       (pf-is-type? pform pf-not-sig)
